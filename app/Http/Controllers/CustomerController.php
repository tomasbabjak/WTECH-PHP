...
class CustomerController extends Controller
{
    public function index(Request $request)
    {
        ...
        $zip = request('zip');
        $token = Session::getId();
        //TODO: aby sa nevytvaral stale novy pouzivatel ale update na zaklade tokenu
        ...
        return view('layout.summary')->with('products', $products)->with('customer', $customer)->with('order',$order)->with('sum', $sum);
    }
    ...
    public function show(Request $request)
    {
        ...
        $customer = Order::create(['cart_id'=>$cart_id, 'user_id'=>$user->id, 'customer_id'=>$user->id, 'transport_id'=>$order->transport_id, 'payment_id'=>$order->payment_id]);
        $cart_items = DB::table('cart_items')->where('cart_id',$cart_id)->get();
